 ...
 *    pgmraw format and saves it to basename%d.png.
...
main(int argc, char **argv)
{
	...
	if (argc>=9&&*argv[8]!='0'){
        ...
        ox=x/17;
        oy=y/15;
        fprintf(stderr,"Input: %i*%i pixels, %lu*%lu dpi, %.1fMB.\n",x,y,dpi*17,dpi*15,(float)lb*y/1048576);
        fprintf(stderr,"Ouput: %i*%i pixels, %lu*%lu dpi, %.1fKB raw data.\n",ox,oy,dpi,dpi,(float)ox*oy/1024);
        ...
        sprintf(string,"%s%d.png",basename,filenumber);
        filenumber++;
        of=fopen(string,"w");
        open_png();
        for (z=oy;z;z--){
